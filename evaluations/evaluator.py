...
def main():
    ...
    parser.add_argument("ref_batch", help="path to reference batch npz file", default=None)
...
def get_ssim_lpips(args):
    ...
    # )
    # TODO: ADD
    from datasets.dataLoader import SEN12MSCR
    gt_lst = []
    test_load = SEN12MSCR("/data2/SEN12MSCR", split="test", region="all")
    for i in range(len(test_load)):
        g, _, _ = test_load[i]
        # g = g[[1,2,3,7],:,:]
        
        gt_lst.append(np.asarray(g))
    gt = np.stack(gt_lst, axis=0)

    ds = torch.utils.data.TensorDataset(
        torch.from_numpy(gt), torch.from_numpy(sample_batch).permute(0, 3, 1, 2)
    ) 
    ...
    for i, (ref, sample) in enumerate(tqdm(dataloader)):
        ...
        with torch.no_grad():
            ...
            lpips_score = lpips(ref[:, [2, 1, 0], :, :], sample[:, [2, 1, 0], :, :])
