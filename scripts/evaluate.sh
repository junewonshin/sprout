...
if [[ $DATASET_NAME == "e2h" ]]; then
    ...
    SPLIT=test
    PREFIX=basefreq_ema_0.9999_340000/sample_340000
    DATA_DIR=data2/SEN12MSCR
    SAMPLE_NAME=samples_7899x256x256x4_nfe${NFE}.npz
fi
...
# SAMPLE_PATH="workdir/sen12mscr_naf_ema_0.9999_400000/sample_400000/split=test/heun/steps=1/samples_"
...
if [[ $DATASET_NAME == "e2h" || $DATASET_NAME == "diode" ]]; then
    ...
    # python evaluations/evaluator.py  $SAMPLE_PATH --metric fid
    python evaluations/evaluator.py "" $SAMPLE_PATH --metric lpips
fi