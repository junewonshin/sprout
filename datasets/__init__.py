...
def load_data(
    ...
    root = data_dir
    trainset = SEN12MSCR(root, split="train", region="all")
    valset   = SEN12MSCR(root, split="val", region="all")

    if include_test:
        testset = SEN12MSCR(root, split="test", region="all")
